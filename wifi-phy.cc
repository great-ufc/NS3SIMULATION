...
WifiPhy::CalculateTxDuration (uint32_t size, WifiTxVector txVector, uint16_t frequency)
{
    ...
    std::string svout="";
    std::ifstream myfile ("Values.txt"); // ifstream = padrão ios:in
    int i = 0;
    if (myfile.is_open())
    {
      ...
      while (! myfile.eof() ) //enquanto end of file for false continua
      {
        ...
        i++;             //e não binário(ios::bin) pega cada linha
      }
    ...
    //if(i<2)
      //NS_LOG_INFO("WifiPhy::CalculateTxDuration - frequency: " << frequency);
